...
#==========================
      ...
      def showSaveHotSpotPos(self):
            ...
            self.tab_widget.insertTab(4,self.createSaveHotspotWidget(),unicode("Save Hotspot"))
            ...
            self.projViewControl.sld.setRange(0,self.projections)
            self.projViewControl.sld.valueChanged.connect(self.projViewControl.lcd.display)
            self.projViewControl.sld.valueChanged.connect(self.nextHotSpotPos)
      ...
      def hotSpotSetChanged(self):
            self.projViewControl.hotSpotSetNumb=self.projViewControl.combo2.currentIndex()
      def nextHotSpotPos(self):
            self.projView.hotSpotNumb=self.projViewControl.sld.value()
            self.projView.setImage(self.data[self.projViewElement,self.projView.hotSpotNumb,:,:])
...
#==========================
      ...
      def runReconstruct(self):
            ...
            self.recon.show()
            self.recon.btn.setText("Reconstruction")
            self.recon.sld.setRange(2/900,900)
      ...
      def convert2array(self):
            ...
            for i in arange(self.projections):
                  ...
                  for j in arange(self.channels):
                        self.data[j,i,:,:-2]=f[self.ImageTag][self.dataTag][j,:,:-2]
...
#####!!!! just temp need to be fixed
      ...
      def showProjections(self):
            ...
            self.tab_widget.insertTab(1,self.createProjWidget(),unicode("Projections"))
